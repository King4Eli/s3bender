...
/// object's Content-Type (as declared on upload) as a small sidecar file under a separate
/// {storage.root}/.meta/{bucket}/{key} tree, so it survives to be replayed on download - without
/// that, browsers have nothing but MIME-sniffing to go on and can't reliably render an &lt;img&gt;/
/// &lt;video&gt;/&lt;audio&gt; element pointed at a presigned URL.
///
...
public class ObjectStorageService
{
    ...
    /// <summary>Streams the request body to disk and returns the resulting object's ETag (hex MD5).</summary>
    public async Task<string> PutObjectAsync(string bucket, string key, Stream body, string? contentType)
    {
        ...
        using (var md5 = MD5.Create())
        {
            ...
            WriteContentType(bucket, key, contentType);
    ...
    public ObjectSummary StatObject(string bucket, string key)
    {
        ...
        return new ObjectSummary(key, info.Length, info.LastWriteTimeUtc, ComputeEtag(path), ReadContentType(bucket, key));
    }
    ...
    public List<ObjectSummary> ListObjects(string bucket, string? prefix)
    {
        ...
        return Directory.EnumerateFiles(dir, "*", SearchOption.AllDirectories)
            ...
            .Select(k =>
            {
                ...
                return new ObjectSummary(k, info.Length, info.LastWriteTimeUtc, ComputeEtag(p), ReadContentType(bucket, k));
            })
            .ToList();
    }

    private void WriteContentType(string bucket, string key, string? contentType)
    {
        var metaPath = MetaPath(bucket, key);
        // Control characters would otherwise be replayed verbatim into a response header on
        // every future download; reject rather than store anything that could smuggle one.
        if (string.IsNullOrWhiteSpace(contentType) || contentType.Length > 255 || contentType.Any(char.IsControl))
        {
            if (File.Exists(metaPath)) File.Delete(metaPath);
            return;
        }
        Directory.CreateDirectory(Path.GetDirectoryName(metaPath)!);
        File.WriteAllText(metaPath, contentType);
    }

    private string ReadContentType(string bucket, string key)
    {
        var metaPath = MetaPath(bucket, key);
        if (!File.Exists(metaPath)) return MediaTypeNames.Application.Octet;
        var value = File.ReadAllText(metaPath).Trim();
        return string.IsNullOrWhiteSpace(value) ? MediaTypeNames.Application.Octet : value;
    }
